...
export default new MessageCommand(
  ...
  async (client, message, args) => {
    ...
    if (
      client.config.Github.repository?.includes('Your github url') ||
      !client.config.Github.repository
    ) {
    ...
    const repo = client.config.Github.repository?.replaceAll('https://github.com/', '')
    const res = await fetch(`https://api.github.com/repos/${repo}/commits`, {
      headers: {
        Authorization: client.config.Github.githubToken
          ? `token ${client.config.Github.githubToken}`
          : ''
      }
    })
      ...
      if (commit.sha.trim().substring(0, 7) === client.BUILD_NUMBER) {
        ...
        collector.on('collect', async (interaction) => {
          if (interaction.customId === 'update.run') {
            ...
            try {
              execSync(
                `git pull https://username:${client.config.Github.githubToken}@github.com/${repo}`
              )
            } catch (e) {
