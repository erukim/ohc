...
import { BaseCommand, BaseContext, Command, SlashCommand } from '../../../typings/structures';
...
export default class CommandManager extends BaseManager {
  ...
  public async slashCommandSetup(
    ...
    this.client.contexts.forEach((command: BaseContext) => {
      slashCommands.push(command.data.name ? command?.data.toJSON() : command.data.toJSON());
    });
  ...
  public async slashGlobalCommandSetup(): Promise<void> {
    ...
    for (const command of slashCommands) {
      ...
      if (category?.find((c) => c.name === command.name)) {
        const supportGuild = this.client.guilds.cache.get(config.devGuild);
        await rest
          .put(
            Routes.applicationGuildCommands(this.client.application?.id!, config.devGuild),
            { body: [command] },
          )
          .then(() => {
            this.logger.info(
  ...
  public async clear(guildId: string) {
    ...
    if (guildId) {
      await rest
        ...
        .then(() => {
          this.logger.info(`The command on server ${guildId} was successful.`)
        });
      return `The command on server ${guildId} was successful.` as string
    } else {
      await rest
        .put(Routes.applicationCommands(this.client.application?.id!), {
          body: {},
        ...
        .then(() => {
          this.logger.info(`Command removed from ${this.client.guilds.cache.size} guilds.`)
        });
      return `Command removed from ${this.client.guilds.cache.size} guilds.`
    }
  }
}
